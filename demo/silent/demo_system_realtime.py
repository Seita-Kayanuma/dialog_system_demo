...
class Main:
    def __init__(self,
        ...
        self.websocket = None  # WebSocketを保持するための変数
        # プロンプトの準備
        # summary.txtを読み込む
    ...
    def run(self):
        ...
        # WebSocket接続を事前に確立
        asyncio.run(self.connect_to_chatgpt())
    ...
    async def connect_to_chatgpt(self):
        # WebSocket接続を確立
    ...
    async def get_chatgpt_response_async(self, recognized_text):
        ...
        await self.websocket.send(json.dumps({
            ...
            "item": {
                "type": "message",
                "role": "user",
                "content": [{"type": "input_text", "text": recognized_text}]
            }
        }))
