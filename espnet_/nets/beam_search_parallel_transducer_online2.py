...
#             return out, out, out
    ...
    def default_beam_search(self, enc_out: torch.Tensor) -> List[Hypothesis]:
        ...
        for t in range(max_t):
            ...
            while True:                
                max_hyp = heapq.heappop(hyps)  # max(hyps, key=lambda x: x.score)
                #hyps.remove(max_hyp)
...
#                 )
                ...
                heapq.heappush(kept_hyps, 
                    Hypothesis(
                        score_p=(max_hyp.score + float(logp[0:1]))*-1,
                ...
                hyps_max = float(hyps[0].score)
                
                #logging.warning(f"#num kept_hyps: {len(kept_hyps)}")
                kept_most_prob = []
                heapq.heapify(kept_most_prob)
                for hyp in kept_hyps:
                    if hyp.score > hyps_max:
                        kept_most_prob.append(hyp)
                    else:
                        break
...
#                 )
        ...
        print("{:.5f}, {:.5f}, {:.5f}".format(search_end-search_start, total_decoder_time, (search_end-search_start)-total_decoder_time))
        return kept_hyps
