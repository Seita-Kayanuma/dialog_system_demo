...
class ContextualBlockDualDelayConformerEncoder2(AbsEncoder):
...
#         ys_r_pad = xs_pad.new_zeros(xs_pad.size())
...
#             ).fill_(1)
        ...
        # random masking at look ahead part
        is_masking = np.random.randn() < self.mask_rate
        if is_masking:
            mask = torch.zeros(xs_pad.narrow(-2, left_idx, self.look_ahead).shape).to(xs_pad.device)
            xs_chunk[:, block_idx, self.block_size + 1 - self.look_ahead : self.block_size + 1] = mask            
        ...
        # random masking at look ahead part
        # is_masking = np.random.randn() < self.mask_rate
#         if is_masking and last_size > self.block_size - self.look_ahead:
#             logging.warning(f"last_size: {last_size}")
#             mask_size = self.block_size - self.look_ahead
#             mask = torch.zeros(xs_pad.narrow(-2, left_idx, self.look_ahead).shape).to(xs_pad.device)
#             xs_chunk[:, block_idx, last_size + 1 - self.look_ahead : last_size + 1] = mask
...
#         ys_r_pad[:, left_idx : cur_hop + self.look_ahead] = ys_chunk[:, block_idx, 1 : cur_hop +self.look_ahead + 1]
...
#             ys_r_pad[:, cur_hop + self.hop_size : cur_hop + self.hop_size + self.look_ahead] = ys_chunk[
#                 :, block_idx, offset + self.hop_size : offset + self.hop_size + self.look_ahead
#             ]
            cur_hop += self.hop_size
            left_idx += self.hop_size
            block_idx += 1
...
#         ys_r_pad[:, cur_hop + self.hop_size : total_frame_num] = ys_chunk[
#             :, block_idx, offset + self.hop_size : total_frame_num - cur_hop + offset, :
#         ]

        if self.normalize_before:
            ys_pad = self.after_norm(ys_pad)
#             ys_r_pad = self.after_norm(ys_r_pad)
...
#         logging.warning(f"xs_chunk shape: {xs_chunk.shape}")
        ...
        ys_chunk_r, _, _, _, past_encoder_ctx_r, _, _ = self.encoders(
            xs_chunk_r, mask_online, True, past_encoder_ctx
        )
        
#        if len(ys_chunk.shape) != 4:
#            ys_chunk = ys_chunk.unsqueeze(0)
