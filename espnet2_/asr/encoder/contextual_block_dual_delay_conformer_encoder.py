...
class ContextualBlockDualDelayConformerEncoder(AbsEncoder):
    ...
    def forward_train(
        ...
        return (ys_pad, ys_r_pad), olens, None
...
#            ys_chunk = ys_chunk.unsqueeze(0)
        ...
        ys_r_pad = xs_pad.new_zeros((xs_pad.size(0), y_r_length, xs_pad.size(2)))
        if n_processed_blocks == 0:
            ys_pad[:, 0:offset] = ys_chunk[:, 0, 0:offset]
            ys_r_pad[:, 0 : offset] = ys_chunk[:, 0, 0 : offset]
        for i in range(block_num):
            cur_hop = i * self.hop_size
            if n_processed_blocks == 0:
                cur_hop += offset
            ...
            if la > 0 and y_r_length > 0:
                ys_r_pad[:, cur_hop : cur_hop + la] = ys_chunk[
                    :, i, offset : offset + la
                ]
            elif y_r_length == 0:
                ys_r_pad = ys_pad
        ...
        return (ys_pad, ys_r_pad), None, next_states
