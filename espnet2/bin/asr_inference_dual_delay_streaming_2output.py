#!/usr/bin/env python3
import argparse
from tools.espnet.nets.batch_beam_search_online_dual_delay_2output import BatchBeamSearchOnlineDualDelay
...
from tools.espnet2.asr.encoder.contextual_block_conformer_encoder import (  # noqa: H301
    ContextualBlockConformerEncoder,
)
from tools.espnet2.asr.encoder.contextual_block_transformer_encoder import (  # noqa: H301
    ContextualBlockTransformerEncoder,
)
from tools.espnet2.asr.encoder.contextual_block_dual_delay_conformer_encoder import (
    ContextualBlockDualDelayConformerEncoder,  # noqa: H301
)
from tools.espnet2.asr.encoder.contextual_block_dual_delay_conformer_encoder2 import (
    ContextualBlockDualDelayConformerEncoder2,  # noqa: H301
)

from espnet2.fileio.datadir_writer import DatadirWriter
from tools.espnet2.tasks.asr import ASRTask
from tools.espnet2.tasks.lm import LMTask
...
class Speech2TextStreaming:
    ...
    def __init__(
        ...
        beam_search = BatchBeamSearchOnlineDualDelay(
            beam_size=beam_size,
            weights=weights,
            scorers=scorers,
            sos=asr_model.sos,
...
#             logging.warning(f"# speech size: {speech.size(0)}")
#             logging.warning(f"# n_frames: {n_frames}, n_residual: {n_residual}")
...
#             logging.warning(f"# wav buffer size: {waveform_buffer.shape}")
...
#         logging.warning(f"# extract_feats output size: {feats.shape}")
...
#                 logging.warning(f"# triming output size: {feats.shape}")
            else:
                feats = feats.narrow(1, 2, feats.size(1) - 4)
...
#         logging.warning(f"# speech shape: {speech.shape}")
...
#         logging.warning(f"# enc shape: {enc[0].shape}")
        
        enc_r = enc[1]
        enc = enc[0]
        
        nbest_hyps, nbest_hyps_r, is_update = self.beam_search(
            x=enc[0],
            #x2=enc[0],
            x2=enc_r[0],
        ...
        ret_r = self.assemble_hyps(nbest_hyps_r)
        if is_final:
            self.reset()
            
        return ret, ret_r, is_update
    ...
    def assemble_hyps(self, hyps):
        ...
        for hyp in nbest_hyps:
            ...
            if token_int[-1] == self.eou:
                token_int = token_int[1:]
                
            else:
                token_int = token_int[1:-1]
            ...
            try:
                if sim_chunk_length == 0:
                    ...
                    for i in range(len(speech) // sim_chunk_length):
                        #frame_id = (max_i - 3 - i)
                        logging.warning("==================================")
                        #logging.warning(f"# {-frame_id} frames from EOU token")
                        logging.warning(f"#InputID: {i}")
                        #logging.warning(f"# data length: {len(speech[i * sim_chunk_length : (i + 1) * sim_chunk_length])}")
                        logging.warning("==================================")
                        results, is_eou, end_block = speech2text(
                            speech=speech[
                    ...
                    if not is_eou:
                        logging.warning("==================================")
                        #logging.warning(f"# 3 frames from EOU token")
                        logging.warning(f"#InputID: {i+1}")
                        #logging.warning(f"# data length: {len(speech[(i + 1) * sim_chunk_length : len(speech)])}")
                        logging.warning("==================================")
                        results, is_eou, end_block = speech2text(
