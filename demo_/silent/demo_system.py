...
# python demo_/silent/old/demo_system.py
...
def callback(indata, frames, time, status):
    global i, audio_num, wav_chunk, asr_buffer, chunk_size, vad_list, vad_binary_list, active_flg
    wav_chunk = np.frombuffer(indata[::, 0], dtype=np.int16)
    wav_chunk = wav_chunk/32767.0
    with torch.no_grad():
        # asr
        # asr_buffer = np.concatenate([asr_buffer, wav_chunk])
        # text = asr.recognize(asr_buffer)
        text = asr.recognize(wav_chunk)
        print(text)
        # if text is not None:
        #     text = asr.convert_kana_to_text(text)
        print(text)
        # spectrogram
        if i == 0:
